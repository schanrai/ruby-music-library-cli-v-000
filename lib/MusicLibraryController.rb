...
def play_song
    ...
    if (1..Song.all.length).include?(input) #checks that the user entered a number between 1 and the total number of songs in the library
      song = Song.all.sort{ |a, b| a.name <=> b.name }[input - 1] #you have to minus the input by 1 to match array numbers
  ...
  def search_artist_by_name
    ...
    input = gets.chomp#turns string number from user input to integer
